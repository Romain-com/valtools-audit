...
async function cartographierActeurs(
  ...
  const [serpMarque, serpTourisme] = await Promise.all([
    trackApiCall({
      ...
      call: () => googleSerp(`"${destination}"`, 10),
    ...
    trackApiCall({
      ...
      apiName: "dataforseo",
      endpoint: "google/serp",
      call: () => googleSerp(`Tourisme ${destination}`, 10),
      estimateCost: () => API_COSTS.dataforseo.serp,
    }).catch(() => []),
  ]);
  ...
  try {
    ...
    const prompt = `Voici les 10 résultats Google trouvés pour la recherche "${destination}" :
${listForLLM}
...
Sortie JSON : { "urlOT": "..." ou null, "urlMairie": "..." ou null, "classement": [{"url": "...", "categorie": "...", "position": N}], "alertes": ["..."] }`;
    ...
    const classification = await trackApiCall({
      auditId,
      apiName: "openai",
      ...
      call: () =>
        askLLMJson<{
          ...
          alertes: string[];
        }>(prompt, "Agis comme un expert SEO."),
      estimateCost: () =>
...
async function auditerPageSpeed(
  ...
  try {
    const psi = await trackApiCall({
      auditId,
      apiName: "pagespeed",
      endpoint: "mobile",
      call: () => runPageSpeedAudit(url, "mobile"),
      estimateCost: () => 0,
    });
    ...
    try {
      const prompt = `Voici les résultats de l'audit technique mobile du site ${url} :
...
export async function runSchemaDigital(
  ...
  const acteurs = await cartographierActeurs(destination, auditId);

  // Étape 2 : Audit PageSpeed sur le site OT (si trouvé)
  let pagespeed: SchemaDigitalOutput["pagespeed"];
  if (acteurs.urlOT) {
    pagespeed = await auditerPageSpeed(acteurs.urlOT, auditId);
  } else {
    pagespeed = {
      score: 0,
      lcp: 0,
