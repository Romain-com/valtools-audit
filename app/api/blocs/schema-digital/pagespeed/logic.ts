// Logique métier — Google PageSpeed Insights
...
/**
 * Analyse les Core Web Vitals pour une liste de domaines (max 3).
 * Les appels sont effectués en parallèle — une erreur sur un domaine ne bloque pas les autres.
 */
export async function executerPageSpeed({ domaines }: { domaines: string[] }): Promise<{
  resultats: ResultatPageSpeed[]
  cout: { nb_appels: number; cout_unitaire: number; cout_total: number }
}> {
  if (!domaines?.length) {
    throw new Error('Paramètre domaines manquant ou vide')
  }

  const apiKey = process.env.PAGESPEED_API_KEY
  if (!apiKey) {
    throw new Error('Variable PAGESPEED_API_KEY manquante')
  }
