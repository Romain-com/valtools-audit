// Logique métier — Analyse du site Office de Tourisme
...
/**
 * Analyse le site OT via OpenAI pour détecter ses fonctionnalités et sa maturité digitale.
 * En cas d'erreur, retourne une analyse partielle avec des valeurs "incertain".
 */
export async function executerAnalyseOT({
  destination,
  domaine_ot,
  titre_ot,
  meta_description_ot,
  ...
  url_ot?: string
}): Promise<AnalyseSiteOT & { cout: { nb_appels: number; cout_unitaire: number; cout_total: number }; erreur?: string }> {
  ...
  const promptUtilisateur = `Tu es expert en marketing digital pour les offices de tourisme français. Réponds UNIQUEMENT avec un JSON valide, sans markdown, sans commentaires.

Site officiel de l'office de tourisme de ${destination} :
