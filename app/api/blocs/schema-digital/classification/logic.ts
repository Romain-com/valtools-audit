// Logique métier — Classification OpenAI des résultats SERP
...
export interface ReponseClassification {
...
/**
 * Classifie les résultats SERP via OpenAI et retourne les sites officiels,
 * le domaine OT principal et la visibilité par intention de recherche.
 */
export async function executerClassification({
  destination,
  tous_resultats,
  par_requete,
}: {
...
}): Promise<ReponseClassification> {
  if (!destination || !tous_resultats?.length) {
    throw new Error('Paramètres destination et tous_resultats requis')
  }
...
⚠️ top3_officiels = uniquement les résultats dont la catégorie commence par "officiel_"`
  ...
  try {
    ...
    // Erreur non bloquante — retourner un résultat vide
