...
export async function POST(req: NextRequest) {
  try {
    ...
    const { destination, tableau_comparatif, insight_gap } = body as {
      destination: string
      tableau_comparatif: TableauComparatif
      insight_gap?: string  // keywords non couverts vs concurrents (depuis siteCompetitors Haloscan)
    }

    if (!destination || !tableau_comparatif) {
      return NextResponse.json(
        { error: 'destination et tableau_comparatif requis' },
        { status: 400 }
      )
    }
