...
export async function POST(req: NextRequest) {
  try {
    ...
    const {
      ...
      destination,       // nom de la destination (pour OpenAI)
      departement,       // code département (pour OpenAI)
      montant_ts,        // montant TS connu en € (pour calibrer OpenAI)
    }: {
    ...
    if (!communes || communes.length === 0) {
      return NextResponse.json({ error: 'communes requis (tableau non vide)' }, { status: 400 })
    }
