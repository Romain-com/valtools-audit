// Logique métier — Apify Instagram
...
/**
 * Récupère les stats du hashtag et les posts récents pour une destination Instagram.
 * Chaque appel Apify est indépendant — une erreur partielle ne bloque pas le retour.
 */
export async function executerInstagram({
  hashtag,
}: {
  hashtag: string
}): Promise<ResultatInstagram & { erreur_stats?: boolean; erreur_posts?: boolean }> {
  if (!hashtag) {
    throw new Error('Paramètre hashtag manquant')
  }

  const token = process.env.APIFY_API_TOKEN

  if (!token) {
    throw new Error('Variable APIFY_API_TOKEN manquante')
  }
