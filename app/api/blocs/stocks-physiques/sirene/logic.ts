// Logique métier — stocks-physiques/sirene
...
/**
 * Récupère les établissements touristiques depuis l'API Recherche Entreprises.
 * @param code_insee - Code INSEE de la commune cible
 */
export async function executerSIRENE({
  code_insee,
}: {
  code_insee: string
}): Promise<RetourStocksSIRENE> {
  if (!code_insee) {
