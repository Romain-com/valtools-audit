// Logique métier — OpenAI classification des keywords
...
/**
 * Exécute la classification OpenAI des keywords.
 * Fusionne le corpus marché avec les positions OT, filtre les patterns évidents,
 * puis classe par batch de 50.
 */
export async function executerClassificationSEO({
  destination,
  keywords_marche,
  keywords_positionnes_ot,
}: {
  destination: string
  keywords_marche: unknown[]
  keywords_positionnes_ot: unknown[]
}) {
  if (!destination || !keywords_marche || !keywords_positionnes_ot) {
    throw new Error('Paramètres destination, keywords_marche et keywords_positionnes_ot requis')
  }
  ...
  const keywords_fusionnes = preparerKeywordsInput(
    keywords_marche as KeywordMarche[],
    keywords_positionnes_ot as KeywordPositionneOT[]
  )
