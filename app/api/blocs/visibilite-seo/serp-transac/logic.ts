// Logique métier — DataForSEO SERP live (Phase B)
...
/**
 * Exécute les appels SERP live pour les keywords transactionnels sélectionnés.
 * Séquentiel pour limiter la charge sur l'API DataForSEO.
 */
export async function executerSERPTransac({
  keywords_classes,
  domaine_ot,
}: {
  ...
  if (!domaine_ot) {
    throw new Error('Paramètre domaine_ot requis')
  }
  ...
  const keywords_selectionnes = selectionnerKeywordsPhaseB(keywords_classes as KeywordClassifie[])
