// Logique métier — Haloscan keywords/overview (marché)
...
/**
 * Exécute l'appel Haloscan market pour une destination.
 * Lance 8 seeds en parallèle couvrant toutes les intentions touristiques.
 */
export async function executerHaloscanMarket({ destination }: { destination: string }) {
  if (!destination) {
